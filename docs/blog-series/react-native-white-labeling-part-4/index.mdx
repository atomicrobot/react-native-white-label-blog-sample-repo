...
title: 'React Native SDK Config: Two-Tier Screens & Render Slots'
author: amanda-scheetz
image: ./hero.webp
excerpt: Design a type-safe React Native SDK config system with BotProvider, two-tier screens, and render slots — clients launch fully branded apps from one config file.
keywords:
  - React Native
  - White Label
  - TypeScript
...
> **In brief**: Configuration is what turns a component library into a white-label SDK. This post builds the full config architecture — typed interfaces, smart defaults, `BotProvider`, two-tier screens (preset for zero-code clients, base for full control), and render slots that let clients inject custom UI without touching SDK code.

We've built the theming system and component library. Now comes the piece that makes white-labeling actually work: the configuration architecture. It's also what determines how fast a new client can go from zero to a branded [mobile app](/services/mobile-app-development/) — and how much SDK maintenance you'll carry long-term.
...
*See this pattern in action: [`examples/acme-health/app/(tabs)/chat.tsx`](https://github.com/atomicrobot/react-native-white-label-blog-sample-repo/tree/main/examples/acme-health/app/(tabs)/chat.tsx)*
...
*See a customized base screen: [`examples/acme-health-advanced/app/(tabs)/chat.tsx`](https://github.com/atomicrobot/react-native-white-label-blog-sample-repo/tree/main/examples/acme-health-advanced/app/(tabs)/chat.tsx)*
...
*See render slots in practice: [`examples/acme-health-advanced/app/(tabs)/settings.tsx`](https://github.com/atomicrobot/react-native-white-label-blog-sample-repo/tree/main/examples/acme-health-advanced/app/(tabs)/settings.tsx)*
...
*See a production auth handler setup: [`examples/acme-health-advanced/app/_layout.tsx`](https://github.com/atomicrobot/react-native-white-label-blog-sample-repo/tree/main/examples/acme-health-advanced/app/_layout.tsx)*
...
If you want to see all of this working together, clone the [sample repository](https://github.com/atomicrobot/react-native-white-label-blog-sample-repo), swap in your own brand colors, and see how fast you can get a branded app running. That's the best way to get a feel for the architecture.

Happy building.

If you're planning a white-label SDK or a multi-brand [mobile app](/services/mobile-app-development/) and want to talk through architecture decisions with a team that's built one, [reach out to us](/contact/) — we'd be happy to dig in.
