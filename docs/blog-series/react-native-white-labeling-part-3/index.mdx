...
title: 'React Native White Labeling: Part 3'
author: amanda-scheetz
image: ./hero.webp
excerpt: Create flexible, theme-aware React Native components — from ThemedText and ThemedView to buttons, cards, and composition patterns that adapt to any brand.
keywords:
  - React Native
  - White Label
  - Mobile Development
...
Tokens and hooks are great, but they don't mean much until components are actually using them. In Part 2, we built the foundation — tokens, hooks, and resolution logic. Now let's build the pieces that turn all of that into real UI.

The goal: components that look native to any brand without anyone touching the code.

{/* TODO: Screenshot — Annotated Showcase screen showing typography, buttons, cards, and inputs with callouts identifying which themed component or token drives each element. */}
![Annotated showcase of themed components — typography variants, button styles, card patterns, and input states](./components-annotated.webp)
...
*See the full implementations: [`ThemedText.tsx`](https://github.com/AcmeOrg/white-label-sdk-sample/tree/main/packages/sdk/src/components/ThemedText.tsx) and [`ThemedView.tsx`](https://github.com/AcmeOrg/white-label-sdk-sample/tree/main/packages/sdk/src/components/ThemedView.tsx)*
...
> **Heads up:** The `ThemedCard`, `ThemedButton`, and `ThemedInput` components discussed below are illustrative patterns — the SDK uses these patterns internally in its screen implementations rather than exporting them as standalone components. You'd build these for your own app using the same `useTheme()` approach. See [`examples/acme-health-advanced/components/`](https://github.com/AcmeOrg/white-label-sdk-sample/tree/main/examples/acme-health-advanced/components/) for working examples.
...
**Platform-specific components** use file extensions: `HapticTab.ios.tsx`, `HapticTab.android.tsx`. Metro bundler automatically picks the right file. This keeps platform differences isolated without conditional logic cluttering your components.

*See [`IconSymbol.tsx`](https://github.com/AcmeOrg/white-label-sdk-sample/tree/main/packages/sdk/src/components/ui/IconSymbol.tsx) for a platform-specific component example.*
